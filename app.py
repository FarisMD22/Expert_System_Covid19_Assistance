    ...
    if st.button(t('btn_diagnose'), type="primary"):
        with st.spinner(t('msg_processing')):
            ...
            if results:
                ...
                st.warning("**Insufficient information for diagnosis**")
                ...
                if (runny_nose or sneezing) and fever:
                    suggestions.append(
                        "You have **nasal symptoms** (runny nose/sneezing) but also **fever**. This is contradictory - allergies/colds don't usually cause fever. Either uncheck fever or check other symptoms.")

                if itchy_eyes and fever:
                    suggestions.append(
                        "You have **itchy eyes** (suggests allergies) but also **fever** (suggests infection). These contradict each other. Allergies don't cause fever.")
...
def page_history():
    ...
    st.markdown("---")
    st.markdown("###  Generate Figures & Analytics")

    # Check if we have data
    if not st.session_state.assessment_history:
        st.info(" No history data. Complete at least one diagnosis first!")
    else:
        # Transform assessment_history to format expected by visualizations.py
        history_for_viz = []

        for assessment in st.session_state.assessment_history:
            # Extract data safely
            diagnosis_data = assessment.get('diagnosis', [])
            risk_data = assessment.get('risk', {})

            # Get diagnosis name and confidence
            if diagnosis_data and len(diagnosis_data) > 0:
                diag_name = diagnosis_data[0][0] if len(diagnosis_data[0]) > 0 else 'Unknown'
                diag_conf = diagnosis_data[0][1] if len(diagnosis_data[0]) > 1 else 0.0
            else:
                diag_name = 'Unknown'
                diag_conf = 0.0
            ...
            viz_entry = {
                'timestamp': assessment.get('timestamp', datetime.now()).strftime('%Y-%m-%d %H:%M:%S'),
                'diagnosis': diag_name,
                'confidence': diag_conf,
                'risk_level': risk_level.capitalize(),  # "low" -> "Low"
                'risk_score': risk_score,
                'symptoms': assessment.get('symptoms', {})
            }
            history_for_viz.append(viz_entry)

        # Show how many records we have
        st.info(f" Found {len(history_for_viz)} assessment(s) ready for visualization")

        # Create buttons
        col1, col2, col3 = st.columns(3)

        with col1:
            if st.button(" Generate All Figures", type="primary", key="gen_all"):
                with st.spinner("Generating comprehensive report..."):
                    try:
                        import visualizations as viz
                        import io

                        fig = viz.generate_comprehensive_report(history_for_viz)
                        ...
                        st.error(" visualizations.py not found. Please add it to your project folder.")
                        st.info("Make sure visualizations.py is in the same directory as app.py")
                    except Exception as e:
                        st.error(f" Error: {str(e)}")
                        st.info("Debug info: Check that visualizations.py has generate_comprehensive_report()")

        with col2:
            if st.button(" Diagnosis Distribution", key="gen_diag"):
                with st.spinner("Generating diagnosis chart..."):
                    try:
                        import visualizations as viz
                        import io

                        fig = viz.generate_diagnosis_pie_chart(history_for_viz)
        ...
        with col3:
            if st.button("Risk Analysis", key="gen_risk"):
                with st.spinner("Generating risk chart..."):
                    try:
                        import visualizations as viz
                        import io

                        fig = viz.generate_risk_distribution_chart(history_for_viz)
